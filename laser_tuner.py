...
freq_piezo_pid = PID(P=900,I=18000.0,D=30.0)
...
def wavelength_pid_loop(wavelength:float, pid:PID,stop_event:Event, stable_event:Event, shg_addr:str, wavemeter_addr:str, piezo_max:float,piezo_min:float,log_file:str, sample_time:float,tol:float):
...
def frequency_pid_loop(frequency:float, pid:PID,stop_event:Event,stable_event:Event, shg_addr:str, wavemeter_addr:str, piezo_max:float,piezo_min:float,log_file:str, sample_time:float,tol:float):
    shg = SHGpro(shg_addr,None)
    wavemeter = WavemeterDFC(wavemeter_addr)
    piezo_offset = shg.get_dl_piezo()
    ...
    with open(log_file,'w') as log:
        ...
        while not stop_event.is_set():
            ...
            # adaptive time stepping
            # print(i,'piezo',output)
            # print(piezo_pid.PTerm , (piezo_pid.Ki * piezo_pid.ITerm) ,(piezo_pid.Kd * piezo_pid.DTerm))
            shg.set_dl_piezo(max(min(piezo_max,output),piezo_min))
            sleep(sample_time)
            curr_frequency = wavemeter.measure_frequency()*1e-12
...
def tune_laser_wavelength(wavelength:float,shg:SHGpro,wavemeter:WavemeterDFC,stop_event:Event,stable_event:Event,sample_time:float=0.05, log_file:str='pid_log.csv',tol:float=0.000001,piezo_pid:PID=wave_piezo_pid):
...
def tune_laser_frequency(frequency:float,shg:SHGpro,wavemeter:WavemeterDFC,stop_event:Event,stable_event:Event,sample_time:float=0.05, log_file:str='pid_log.csv',tol:float=0.000001,piezo_pid:PID=freq_piezo_pid):
    # start shg crystal temperature
    shg.set_dl_current(200.0)
    shg.set_dl_piezo(70.0)
    keywords = {
...
if __name__ == "__main__":
    ...
    target_freq = nm2thz(1120)
    loop_handle = tune_laser_frequency(target_freq,shg,wavemeter,stop_event,stable_event)
